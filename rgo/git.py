...
class Git(object):
    ...
    def get_describe_long(self, ref):
        """
        Run git describe --tags --long <ref>
        Return the a tuple with the following data:
          * desc: raw `git desribe --tags --long <ref>` output
          * tag: last available tag in commits <= ref (parsed from desc)
          * commits: number of commits between the tag and the ref (parsed from desc)
          * hash: hash of the ref (parsed from desc)
        """
    ...
    def get_rpm_changelog(self):
        ...
        ref_tag, ref_commits, _ = self.get_describe_long(self.ref)
        ...
        if ref_commits == 0:
            ...
            tag_tag, _, _ = self.get_describe_long(ref_tag + "^")
        else:
            # there are commits between ref and the latest tag -> use the tag
            tag_tag = ref_tag

        cmd = ["git", "log", "--pretty=format:- [%h] %s (%an)", "{0}..{1}".format(tag_tag, self.ref)]
